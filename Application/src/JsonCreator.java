...
public class JsonCreator {// this class takes the array of linked
							// hash-maps<string,object>(key(id,name,url),value) and iteratively adds values
							// to the json object

	public List<LinkedHashMap<String, Object>> cache = new ArrayList<LinkedHashMap<String, Object>>();// cache will
																										// contain
																										// entries
																										// already added
																										// to JSON
	...
	public JSONObject createSecondRow(List<LinkedHashMap<String, Object>> list, JSONObject jsonObject) {
		for (LinkedHashMap<String, Object> element : list) {
			...
			if (level == 2 && !cache.contains(element)) {
				((List<LinkedHashMap<String, Object>>) ((HashMap) ((ArrayList) jsonObject.get("Categories")).get(0))
						.get("Children")).add(element);
				cache.add(element);
			}
		}
		return jsonObject;
	...
	public JSONObject createThirdRow(List<LinkedHashMap<String, Object>> list, JSONObject jsonObject) {
		for (LinkedHashMap<String, Object> parent : ((ArrayList<LinkedHashMap<String, Object>>) ((HashMap) ((ArrayList) jsonObject
				.get("Categories")).get(0)).get("Children"))) {
	...
	private boolean isChild(LinkedHashMap<String, Object> parent, LinkedHashMap<String, Object> child) {// this method
																										// will return
																										// true if the
																										// child in the
																										// sequence is a
